...
| `context-menu` (shadcn)              | `SealContextMenu` — right-click / long-press context menu; pass `trigger` for the interactive area; compose `.Item`, `.Separator`, `.Label`, `.CheckboxItem`, `.RadioGroup`, `.RadioItem`, `.Sub`, `.SubTrigger`, `.SubContent`, `.Shortcut` as children |
